...
|----|----|----|----|
| A | state-only predictor | Bernoulli | design-effect-zero reference standardization |
| A\* | same as A | same as A | A after fixed sampling-noise correction |
| B | full state + stratum + nested-PSU predictor | Bernoulli + design mixture | realized-design-composition standardization |
...
A is a reference standardization under fitted stratum and PSU effects
set to zero. It is not, without additional identification assumptions, a
causal or policy intervention.
...
The A-versus-B gap is sensitivity to whether the fitted realized design
composition is retained. It can reflect fitted composition,
nonlinearity, weighting, and model structure; it is not specific to
