...
- Updated the scope and limitations documents to report a simulation
  study of 400 fits. Under the design family used in the accompanying
  article, central 90% intervals covered their targets at nominal rates
  for the primary estimands: pooled coverage 0.900, 95%
  cluster-bootstrap interval 0.871 to 0.928.
...
- Sharpened the guidance on equivalence: the number of domains, not the
  number of observations, is the binding constraint on resolving small
  gaps.
...
is the version that implements the model given in Appendix A of that
article.
...
frequentist coverage has not been established; see
[`vignette("scientific-limitations")`](https://joonho112.github.io/bhfvar/articles/scientific-limitations.md).
