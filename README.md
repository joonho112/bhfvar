...
frequentist coverage has not been established, and a sandwich-type variance
adjustment is not implemented in this version. A* conditions on the sampling
variances as fixed and does not propagate their estimation uncertainty. The
model is intercept-only with a binary outcome. The article's restricted-data
NSECE application is not reproduced by this package.
