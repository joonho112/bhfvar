---
title: "Model Diagnostics and Convergence Checking"
author: "JoonHo Lee"
date: "`r Sys.Date()`"
description: >
  A comprehensive guide to assessing model convergence, diagnosing problems, 
  and validating results from bhfvar models.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Model Diagnostics and Convergence Checking}
...
```{r basic-diagnostics, eval=FALSE}
library(bhfvar)
...
**What they are**: Divergent transitions occur when the Hamiltonian Monte 
Carlo (HMC) sampler encounters regions of high curvature that it cannot 
navigate accurately.

**How to check**:
```{r divergences, eval=FALSE}
...
**What it is**: R-hat compares variance within chains to variance between 
chains. Values near 1 indicate chains have mixed well.
...
```{r bayesplot-diagnostics, eval=FALSE}
library(bayesplot)
...
```{r weight-sensitivity, eval=FALSE}
...
**Diagnosis**:
```{r diagnose-divergences, eval=FALSE}
...
2. Reparameterize (bhfvar already uses non-centered)
3. Check for data issues (extreme weights, sparse domains)
...
**Solutions**:
1. Run longer
2. Use thinning (set `thin = 2` or higher)
