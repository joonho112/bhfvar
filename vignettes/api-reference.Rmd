---
title: "API Reference: Complete Function Documentation"
author: "JoonHo Lee"
date: "`r Sys.Date()`"
description: >
  Comprehensive documentation of all exported functions in the bhfvar package, 
  with detailed parameter descriptions and usage examples.
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{API Reference: Complete Function Documentation}
...
This vignette provides comprehensive documentation of all functions exported 
by the bhfvar package. Functions are organized by workflow stage:
...
An object of class `stanmodel` that can be passed to `bhf_fit()`.
...
- `sampling_variances`: Vector of $\hat{V}_s$ (if `use_deattenuation = TRUE`)
...
| Argument | Type | Default | Description |
|----------|------|---------|-------------|
| `data` | bhf_data | — | Output from `prepare_bhf_data()` |
| `model` | stanmodel | — | Output from `compile_bhf_model()` |
...
available cores). Convergence diagnostics are automatically computed 
and warnings issued if problems are detected.
...
| Argument | Type | Default | Description |
|----------|------|---------|-------------|
| `fit` | bhf_fit | — | Output from `bhf_fit()` |
...
- **Marginal** estimates integrate out within-domain (PSU) variation using 
  the Zeger adjustment. These are appropriate for population inference.
  
- **Conditional** estimates are $\text{logit}^{-1}(\alpha + u_s)$ without 
  integration. These represent the domain-specific propensity.
...
| Argument | Type | Default | Description |
|----------|------|---------|-------------|
| `stan_data` | list | — | Stan data list from `prepare_bhf_data()` |
...
|-------|----------|-------|--------|
| 1. Compile | `compile_bhf_model()` | — | `stanmodel` |
| 2. Prepare | `prepare_bhf_data()` | data.frame | `bhf_data` |
| 3. Fit | `bhf_fit()` | bhf_data, stanmodel | `bhf_fit` |
| 4a. Variance | `variance_decomposition()` | bhf_fit | list |
| 4b. Domains | `domain_estimates()` | bhf_fit | data.frame |
| 4c. Overall | `overall_estimate()` | bhf_fit | list |
| 4d. LOO | `log_lik()` | bhf_fit | matrix |
